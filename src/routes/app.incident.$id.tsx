  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2 border-primary/30 bg-primary/5 fs-fade-up relative overflow-hidden">
          <CardHeader>
          ...
          <CardContent>
            <p className="text-sm leading-relaxed fs-fade-up" style={{ animationDelay: "120ms" }}>
            ...
            <div className="mt-4 space-y-1.5 fs-fade-up" style={{ animationDelay: "260ms" }}>
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Hypothesis confidence</span>
                <span className="font-mono font-medium">{Math.round(incident.confidence * 100)}%</span>
              </div>
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-primary to-info transition-[width] duration-[1200ms] ease-[cubic-bezier(0.4,0,0.2,1)]"
                  style={{ width: `${Math.round(incident.confidence * 100)}%` }}
                />
              </div>
            </div>
            <div className="mt-4 flex flex-wrap gap-2 fs-fade-up" style={{ animationDelay: "380ms" }}>
      ...
      <Card>
        ...
        <CardContent>
          <ol className="relative space-y-4 border-l border-border pl-5 fs-stagger">
            {evidenceTimeline.map((e, i) => {
              const Icon = kindIcon[e.kind as keyof typeof kindIcon] ?? Activity;
              const isActive = i === evidenceTimeline.length - 1;
              return (
                <li key={i} className="relative group">
                  <span className={cn(
                    "absolute -left-[1.45rem] flex h-5 w-5 items-center justify-center rounded-full bg-background ring-4 ring-background border transition-all duration-200",
                    isActive ? "border-primary fs-glow-active" : "border-border group-hover:border-primary/60"
                  )}>
                    <Icon className={cn("h-3 w-3 transition-colors", isActive ? "text-primary" : "text-muted-foreground group-hover:text-primary")} />
                  </span>
